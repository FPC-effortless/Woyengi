...
| Action surface | `public.actionSurface[]` | `PublicActionSpec[]` | Preserve public action identity/name, logical `systemRef`, normalized `parameterNames`, and action kind. Map READ/WRITE/EXECUTE/COMMUNICATE/ESCALATE/SUBMIT one-to-one to the lowercase Veritas enum. Public action names are unique. No hidden action/effect may be used to enrich or widen this surface. |
| Action cost | optional `public.actionSurface[].cost` | `PublicActionSpec.cost` where representable | Preserve visibility and magnitude only through an explicit lossless mapping. Woyengi cost is `{amount,currency}` while current Veritas action cost is an integer unit; non-integral, multi-currency, or otherwise incompatible semantics must fail closed or remain structured adapter metadata rather than being silently converted. |
| Permitted systems | public `actionSurface[].systemRef` values | `TaskContract.permitted_systems` | Derive only from public logical system references. The namespace-to-Veritas-string projection must be deterministic and reversible in adapter metadata. Never infer permission from evaluator-private effects. |
...
| Budget | portable outcome budget | `HiddenOracle.max_cost` / `max_tool_calls` when evaluator-enforced; public constraint/metadata when agent-visible | Enforcement placement must preserve visibility. A private evaluator budget must not become a new public hint; a public budget must remain public as well as enforced. Woyengi monetary cost/attempt semantics and Veritas integer cost/tool-call semantics are not assumed equivalent; unit conversion must be explicit and lossless or fail closed. |
...
| Hidden action effects | `private-evaluator.hiddenEffectRefs` plus private resolved material | `HiddenOracle.action_effects` | Evaluator only; action names must correspond to declared public actions. Hidden effects do not widen the public action surface or supply missing public system/parameter metadata. |
...
4. Public action system references, parameter names, descriptions or costs may not be enriched from hidden effect knowledge.
...
- public action system/parameter semantics are deterministic;
...
13. a missing public action system or parameter declaration is filled from a hidden effect, provider default or heuristic;
14. two Woyengi actions with the same public name become an ambiguous Veritas action surface;
15. a Woyengi public monetary/action cost is silently rounded or reinterpreted as Veritas integer cost units.
