...
| 1-6, 147 | One Woyengi product, one workspace model, shell, Work and software loops | Account/workspace, durable Work, Composer/App contracts, and unified shell are functional local slices | Durable production composition and self-hosted multiplayer remain open |
...
| 30-42, 126, 132 | Semantic history, objects, Work/Activity, multiplayer, conflicts | Bitemporal records/relations/history, causal Work journals, participants, handoffs, optimistic concurrency, resumable events, and ephemeral presence are functional contracts | Durable websocket/Postgres two-client composition remains WYG-019B |
...
| 67-78, 135-140 | Capabilities, execution manifest/journal, compute/nodes, local/self-host | Governed execution contracts, JSON-backed offline personal runtime, hosted compute-node contract, local storage, Compose topology, and CLI exist | SQLite CLI start, Postgres two-client serve, real sandbox, and physical remote-node transport remain |
...
- The local JSON ledger is single-process/single-writer; team/server mode needs a database transaction provider preserving atomic bundle and causal-sequence semantics.
- Legacy ledgers can be deterministically backfilled only from their persisted order; historical append order discarded by older timestamp/ID sorting is unrecoverable.
